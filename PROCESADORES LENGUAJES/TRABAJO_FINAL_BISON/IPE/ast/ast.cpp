...
{
	/*int result = 0;
	std::cout<<this->_left->getType()<<std::endl;	
	std::cout<<this->_left->evaluateCadena()<<std::endl;	
...
std::string lp::ConcatNode::evaluateCadena()
{
	std::string result = "";
	...
	{
		warning("Runtime error: the expressions are not string for ", "CONCAT");
	}
...
bool lp::GreaterThanNode::evaluateBool() 
{
	...
	if (this->getType() == BOOL)
	{
		...
		if(this->_left->getType() == CADENA){
		
			std::string leftCadena, rightCadena;
			leftCadena = this->_left->evaluateCadena();
			rightCadena = this->_right->evaluateCadena();
			
			result = (leftCadena > rightCadena);
		}
	}
	else
	{
...
void lp::read_stringStmt::print() 
{
  std::cout << "Leer_cadenaStmt: "  << std::endl;
  std::cout << " Leer_cadena (" << this->_id << ")";
  std::cout << std::endl;
}


void lp::read_stringStmt::evaluate() 
{   
	std::string value;
	std::cout << BIYELLOW; 
	std::cout << "Insert a cadena --> " ;
	std::cout << RESET; 
	
	std::getline(std::cin,value);
	if(value.length()!=0){
	if(value[0]!='\'' or value[value.length()-1]!='\'')
	{	
		execerror("Lexical ERROR: 'cadena' ",value);
		return;
	}
	
	
	
		for(unsigned int i=0; i< value.length()-1; i++){
			value[i] = value[i+1];
...
///////////////////////////////////////////////////////////////////////////////////////////////
void lp::print_stringStmt::print() 
{
  std::cout << "print_stringStmt: "  << std::endl;
  std::cout << " print ";
  this->_exp->print();
  std::cout << std::endl;
}
...
void lp::print_stringStmt::evaluate() 
{
...
void lp::IfStmt::print() 
{
  ...
  // Consequent
  for(std::list<Statement*>::iterator i= this->_list1->begin(); i != this->_list1->end(); i++){
  	(*i)->print();
 ...
 // The alternative is printed if exists
  if (this->_list2 != NULL)
	  for(std::list<Statement*>::iterator i= this->_list2->begin(); i != this->_list2->end(); i++){
  		(*i)->print();
...
void lp::ForStmt::evaluate() 
{
 std::list<Statement *>::iterator stmtIter;
	...
	lp::NumericVariable *paso = NULL;

	  if(this->_step != NULL){
		paso =  new lp::NumericVariable(this->_id,VARIABLE,NUMBER,this->_step->evaluateNumber());
	  }else{
		paso =  new lp::NumericVariable(this->_id,VARIABLE,NUMBER,1.0);
	  }

	  if (n->getValue() > this->_to->evaluateNumber() && paso->getValue() < 0)
	  {
	  	
		for(n->getValue(); n->getValue() >= this->_to->evaluateNumber(); n->setValue(n->getValue() + paso->getValue())){
			for (stmtIter = this->_stmt->begin(); stmtIter != this->_stmt->end(); stmtIter++) 
	  		{
	    		(*stmtIter)->evaluate();
	  		} 
	  ...
	  else if (n->getValue() < this->_to->evaluateNumber() && paso->getValue() > 0)
	  {
