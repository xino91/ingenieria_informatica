...
class ConcatenacionNode : public StringOperatorNode 
{
  public:
/*!		
	\brief Constructor of ConcatenacionNode uses StringperatorNode's constructor as members initializer
	\param L: pointer to ExpNode
	\param R: pointer to ExpNode
  ...
  IfStmt(ExpNode *condition, std::list<Statement *> *stmtList)
	{
		...
		this->_stmtList1 = stmtList;
		this->_stmtList2 = NULL;
	}
...
*/
  IfStmt(ExpNode *condition, std::list<Statement *> *stmtList1, std::list<Statement *> *stmtList2)
	{
		this->_cond = condition;
		this->_stmtList1 = stmtList1;
		this->_stmtList2 = stmtList2;
	}
...
class WhileStmt : public Statement 
{
 private:
  ...
  std::list<Statement *> *_stmtList; //!< List of statments of the body of the while loop
...
class UntilStmt : public Statement 
{
 private:
  ExpNode *_cond; //!< Condicion of the while statement
  std::list<Statement *> *_stmtList; //!< List of statments of the body of the until loop
...
*/
  UntilStmt(ExpNode *condition, std::list<Statement *> *stmtList)
	{
		this->_cond = condition;
		this->_stmtList = stmtList;
	}
...
{
 private:
  std::list<Statement *> *_stmtList; //!< List of statments of the body of the for loop
  ExpNode *_from; //!< Start of the loop
  ExpNode *_until; //!< End of the loop
  char* _var; //!< Variable for iterating
  double _paso; //!< Loop jump

  public:
/*!		
	\brief Constructor of  ForStmt
	\param variable: VariableNode which saves the iteration number
	\param from: NumberNode where the loop starts
	\param until: NumberNode where the loop ends
	\param stmtList: Statements of the body of the loop 
	\param paso: Loop jump
	\post  A new ForStmt is created with the parameters
*/
  ForStmt(char* variable, ExpNode *from, ExpNode *until, std::list<Statement *> *stmtList, double paso = 1)
	{
		this->_var = variable;
		this->_from = from;
		this->_until = until;
...
/*!	
  \class   MoveStmt
  \brief   Definition of atributes and methods of MoveStmt class
  \note    MoveStmt Class publicly inherits from Statement class 
		   and adds its own print and evaluate functions
*/
class MoveStmt: public Statement 
{
 private:
  double _x; //!< X value for the move
  double _y; //!< Y value for the move

 public:
/*!		
	\brief Constructor of MoveStmt 
	\param x: position x for the mouse move
	\param y: position y for the mouse move
	\post  A new MoveStmt is created with the parameter
*/
  MoveStmt(double x, double y)
	{
		this->_x = x;
		this->_y = y;
	}
