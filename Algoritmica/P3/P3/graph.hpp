  ...
  /**@{*/
     ...
     */
     GraphNode(size_t label, T const& item):
         _label(label), _item(item), _visited(false), _visitas(0)
     {}
...
public:
  ...
  */
  ...
  bool has_other(NodeRef const& x) const
  {
  	if(x == _u){
  		if(_v.get() == nullptr){ 
     		return false;
...
public:
    ...
    */
                       ...
                       NodeRef& v) const
    {
		...
		if(_a_mat[u->label()][v->label()] < std::numeric_limits<float>::infinity()){
	     ...
	     else if( _a_mat[v->label()][u->label()] < std::numeric_limits<float>::infinity()) {
	     	return true;
	     }
	     return false;
    }
	...
	std::vector<size_t> adjacent (size_t u)
	{
		std::vector<size_t> adyacentes;
		for(size_t nodo=0; nodo<_a_mat.size(); nodo++){
			if(_a_mat[u][nodo] < std::numeric_limits<float>::infinity() or _a_mat[nodo][u] < std::numeric_limits<float>::infinity()){
  ...
  /** @{*/
    ...
    /*     	EdgeRef minimo(WGraph<T> & g){
         		...
         		g.goto_first_node();
		    	menor = g.current_edge()->item();
		    	while(g.has_current_node()){
				g.goto_first_edge();
				while(g.has_current_edge()){
					if(menor > g.current_edge()->item()){ 
						std::cout << g.current_edge()->first()->label() << std::endl;
						std::cout << g.current_edge()->second()->label() << std::endl;
						std::cout << g.current_edge()->first()->item() << std::endl;
						std::cout << g.current_edge()->second()->item() << std::endl;
						std::cout << g.current_edge()->item() << std::endl << std::endl;

						menor = g.current_edge()->item();
						std::cout << "Menor: " << menor << std::endl << std::endl;
						edgeref._u = g.current_edge()->first()->label();
				     	edgeref._v = g.current_edge()->second()->label();
				     	edgeref._item = g.current_edge()->item();
					}
					g.goto_next_edge();
				 }
				 g.goto_next_node();
    ...
    /** @{ */
        ...
        void get_weight(NodeRef u, NodeRef v, float new_w)
        {
            assert(has(u));
            assert(has(v));
            _a_mat[u->label()][v->label()] = new_w;
