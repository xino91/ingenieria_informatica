...
class Municipio
{
  ...
  //! \name Atributos privados de la clase Municipio
	...
	*/
	...
	{
		int valordevuelto;
		valordevuelto = this->getHombres() + this->getMujeres();
		
			#ifndef NDEBUG
			assert(( valordevuelto == this->getHombres() + this->getMujeres()));
			#endif
	...
	*/
	inline Municipio & operator=(Municipio const &objeto)
	{
		...
		//std::cout<<"operador = municipio"<<std::endl;
		if (this != &objeto) 
		{
			this->setCodigoPostal(objeto.getCodigoPostal());
			this->setNombre(objeto.getNombre());
		...
		//std::cout<<"operador = municipio saliendo"<<std::endl;
	...
	*/
	inline bool operator == (Municipio const &objeto) const
	{
		...
		valordevuelto= this->getNombre() == objeto.getNombre() ;
		
			#ifndef NDEBUG
			assert( valordevuelto == (getNombre() == objeto.getNombre()));
			#endif
	...
	*/
	inline bool operator < (Municipio const &objeto) const
	{
    		...
    		valordevuelto = this->getNombre() < objeto.getNombre();
    			...
    			#ifndef NDEBUG
    			assert( valordevuelto == (this->getNombre() < objeto.getNombre()));
    			#endif
