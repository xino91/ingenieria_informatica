...
class Fecha 
{
  //! \name Atributos privados de la clase Fecha
	...
	inline Fecha(int dia=1, int mes=1, int agno=1)
	{
		...
		this->setAgno(agno);
  		...
  		// Se comprueba la postcondición
			...
			assert(getDia() == dia and getMes() == mes and getAgno() == agno);
		#endif //NDEBUG
	}
	...
	*/
	...
	{
		//std::cout << objeto.getDia() << "-" << objeto.getMes() << "-" << objeto.getAgno() << std::endl;
		#ifndef NDEBUG
	...
	*/
	inline bool operator == (Fecha const &objeto) const
	{
	   bool valor;
		...
		valor = (this->getDia() == objeto.getDia() and  
          	    this->getMes() == objeto.getMes() and
         		    this->getAgno() == objeto.getAgno());
         		    
             #ifndef NDEBUG
	  		// Se comprueba la postcondición
			assert(valor == (this->_dia == objeto.getDia() and 
			                 this->_mes == objeto.getMes()   and 
						  this->_agno == objeto.getAgno()));
			#endif //NDEBUG
		return valor;
	}
	...
	inline bool operator!= (Fecha const & objeto) const {
		...
		if(this->getDia() != objeto.getDia() or this->getMes() != objeto.getMes() or this->getAgno() != objeto.getAgno()){
